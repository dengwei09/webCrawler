...
class WeiboSearchLoginFailure(Exception): pass


class WeiboSearchLogin(object):
    ...
    def login(self, username=None, pwd=None, cookie_filename=None):
        ...
        #the dictionary parameters for post request
        ...
        if cookie_filename is not None:
            ...
            self.cj.save()
        finalPage = HTML.fromstring(indexPage)
        if len(finalPage.xpath("//div[@class='ut']")) != 0:
            return True, self.cj,finalPage
        else:
            return False, self.cj,finalPage
